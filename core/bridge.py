...
class Bridge:
    ...
    FORBIDDEN_PATTERNS = [
        ...
        r"rm\s+/-rf|format|dd",          # 危险系统命令
        r"http://|https://.*\?.*=",      # 防止恶意URL参数
    ]
    ...
    @classmethod
    def get_recent_requests(cls, limit: int = 10) -> List[Dict]:
        """获取最近的请求日志"""
        return cls._request_history[-limit:]
    ...
    def add_allowed_tool(cls, tool_name: str):
        ...
        if tool_name not in cls.ALLOWED_TOOLS:
            ...
            logger.info(f"✅ 工具 {tool_name} 已添加到白名单")
