...
def save_market_data(data: StandardMarketData):
    ...
    try:
        ...
        # 添加日期标记
        save_dict = data.dict()
        save_dict['_cache_date'] = get_today_str()
...
def load_market_data() -> StandardMarketData:
    ...
    try:
        ...
        # 检查是否为今日数据（可选，如果不要求必须今日则注释）
        # cache_date = data_dict.get('_cache_date')
        # if cache_date != get_today_str():
        #     logger.info(f"📅 缓存日期 {cache_date} 非今日，不使用")
        #     return None
...
class RealDataAdapter:
    def __init__(self, phase: str = "pre"):
        ...
        self._rate_limiter = RateLimiter(max_calls=40, period=60)  # 更保守
        self._index_close = 0
        self._index_pct = 0
    ...
    def fetch_all(self) -> StandardMarketData:
        ...
        if cached is not None:
            logger.info("📂 使用缓存市场数据（跳过网络请求）")
            self.data_source = "Cache"
            # 从缓存中恢复大盘数据（如果有）
            if hasattr(cached, '_index_data'):
                self._index_close = cached._index_data.get('close', 0) if hasattr(cached, '_index_data') else 0
            return cached

        # ✅ 2. 尝试 AKShare（快速超时）
        try:
            logger.info("📊 使用 AKShare 获取行业数据...")
            self.data_source = "AKShare"
            ...
            logger.warning(f"⚠️ AKShare 失败 ({e})，尝试备用...")
    ...
    def _fetch_from_akshare(self) -> StandardMarketData:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:  # 串行执行，避免服务器压力
            future_to_name = {executor.submit(self._fetch_single_sector, name): name for name in SECTOR_NAMES}
            for future in future_to_name:
                name = future_to_name[future]
                try:
                    result = future.result(timeout=6)  # ⏱️ 6秒超时
                    sectors.append(result)
                except FuturesTimeoutError:
                    logger.warning(f"⚠️ {name} 获取超时（6s），使用随机值")
