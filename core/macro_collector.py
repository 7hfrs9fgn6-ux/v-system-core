...
# -*- coding: utf-8 -*-
"""
宏观数据采集模块（永久缓存，按日期自动刷新）
- 数据永久存储，每天首次运行时自动获取，之后所有阶段共享缓存
- 移除 force_refresh 参数，改为自动检查缓存日期
- 缓存包含 _cache_date 字段，用于判断是否今日数据
"""
...
class MacroCollector:
    ...
    def get_macro_snapshot(self) -> Dict:
        """
        获取宏观数据快照，永久缓存，每天首次运行时自动刷新
        返回：包含美股、亚太、欧洲、大宗、汇率、A50 的数据
        """
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, 'r') as f:
                    cached = json.load(f)
                # 检查是否今日数据
                if self._is_today_data(cached):
                    logger.info(f"✅ 使用今日缓存宏观快照: {cache_file}")
                    return cached.get('data', {})
                else:
                    logger.info("📅 缓存存在但非今日数据，刷新...")
            except Exception as e:
                logger.warning(f"缓存读取失败，刷新: {e}")

        # 2. 缓存不存在或非今日数据，重新获取
        logger.info("📊 刷新宏观数据（今日首次）...")
        result = {
    ...
    # 辅助：检查缓存是否为今日数据
    # ============================================================
    def _is_today_data(self, cached: Dict) -> bool:
        """检查缓存是否包含今日数据"""
        if '_cache_date' in cached:
            return cached['_cache_date'] == self._today
        # 兼容旧格式
        if '_cache_time' in cached:
            try:
                cache_date = datetime.fromisoformat(cached['_cache_time']).strftime("%Y-%m-%d")
                return cache_date == self._today
            except:
                pass
        return False
